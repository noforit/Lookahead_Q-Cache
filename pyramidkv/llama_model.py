...
def llama_attn_forward_StreamingLLM(
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
        # print(kv_seq_len)
        # print(past_key_value.get_usable_length(kv_seq_len, self.layer_idx))
...
def LlamaAttention_forward_fullkv(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def LlamaFlashAttention2_forward_LAQ(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if isinstance(past_key_value, DynamicCache):
            ...
            key_states, value_states = past_key_value.update(query_states, key_states, value_states, self.layer_idx, self.config.window_size, cache_kwargs)
...
def get_softmax_max_tokenid(logits, tokenizer):
    # print(logits.shape)
...
from pyramidkv.pyramidkv_utils import SnapKVCluster, StreamingLLMKVCluster, H2OKVCluster
from pyramidkv.pyramidkv_utils import DynamicCache_LAQ
...
def select_KV_lookahead_stage(full_past_kv, config):
    ...
    if method.lower() in ["snapkv","pyramidkv","h2o","cam", "l2norm", "adakv", "headkv", "think"]:
    ...
    for layer_idx in range(len(full_past_kv)):
        if method.lower() == 'snapkv':    
            methodCluster = SnapKVCluster
        if method.lower() == 'streamingllm':    
            methodCluster = StreamingLLMKVCluster
        if method.lower() == 'h2o':    
            methodCluster = H2OKVCluster
        ...
        key_states_compress, value_states_compress = cluster.update_kv(full_past_kv.key_cache[layer_idx], full_past_kv.temp_local_window_query[layer_idx], full_past_kv.value_cache[layer_idx], None, None)
        stage1_decode_kv.update(full_past_kv.temp_local_window_query[layer_idx], key_states_compress, value_states_compress, layer_idx, 16)
...
def select_KV_decoding_stage(full_past_kv, new_kv, lookahead_size, method, max_capacity_prompts, all_query, stage2_window_sizes):
    method = 'snapkv' # The cache implementation of LAQ is temporarily written in SnapKVCluster.
    kernel_sizes = 7
    pooling = "maxpool"
    if method.lower() in ["snapkv","pyramidkv","h2o","cam", "l2norm", "adakv", "headkv", "think"]:
    ...
    for layer_idx in range(len(full_past_kv)):
        ...
        key_states_compress, value_states_compress = cluster.update_kv_LAQ(full_past_kv.key_cache[layer_idx], new_kv.temp_local_window_query[layer_idx], full_past_kv.value_cache[layer_idx], None, None)
...
def LlamaForCausalLM_forward_LAQ(
    ...
    def calc_logits(self, input_ids, attention_mask, position_ids, past_key_values, cache_position):
    ...
    if kv_len == 0: 
        ...
        past_key_values = DynamicCache_LAQ()
        ...
        id0 = get_softmax_max_tokenid(logits, self.config.tokenizer)
        new_token = [id0]
        ...
        while len(new_token) <= self.config.lookahead_size:
            ...
            position_ids = torch.tensor([[position_ids[0][-1]+1]], dtype=torch.long, device='cuda:0')
            cache_position = torch.tensor([cache_position[-1]+1], dtype=torch.long, device='cuda:0')
            outputs, logits = calc_logits(self, input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values, cache_position=cache_position)
        ...
        outputs.past_key_values = select_KV_decoding_stage(full_past_kv, past_key_values, len(new_token), self.config.lookahead_method, self.config.max_capacity_prompts, self.config.all_query, self.config.stage2_window_sizes)
        logits = ori_logits
        full_past_kv = None
        past_key_values = None
