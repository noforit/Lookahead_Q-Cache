...
def extract_after_is(text):
    if 'eat a sandwich and sit in dolores park on a sunny day' in text:
        return 'eat a sandwich and sit in dolores park on a sunny day.'
    else:
        return text
...
def main():
    ...
    for file in json_files:
        with open(file, 'r') as f:
            ...
            ori_model_response = json_data.get("model_response", None).lower()
            model_response = extract_after_is(ori_model_response)
    ...
    f = plt.figure(figsize=(20, 8))  # Can adjust these dimensions as needed
    heatmap = sns.heatmap(
        pivot_table,
        vmin=0, vmax=1,
        cmap=cmap,
    ...
    # plt.title(f'Lookahead Query-Cache     KV Cache Size=64', fontsize=18)  # Adds a title
    plt.title(f'H2O     KV Cache Size=64', fontsize=18)  # Adds a title
    # plt.title(f'Lookahead Query-Cache   KV Cache Size=128 \nPressure Testing {model_name_} \nFact Retrieval Across Context Lengths ("Needle In A HayStack")', fontsize=18)  # Adds a title
    ...
    save_path = "./results_needle/img/%s.pdf" % model_name
